...
contract SportCrypt {
    ...
    enum Status {
        OK,//Trade was created successfully.
        MATCH_FINALIZED,//Cannot trade after a match has been finalized.
        ORDER_EXPIRED,//The block this transaction was mined in has a block.timestamp greater than the order's expiry.
        ORDER_MALFORMED,//The order was constructed incorrectly.
        ORDER_BAD_SIG,//The order's signature does not match the signature of the order creator.
        AMOUNT_MALFORMED,//An amount greater than MAX_SANE_AMOUNT was provided.
        SELF_TRADE,//The sender and the order creator are the same account.
        ZERO_VALUE_TRADE //The trade amount was so small that, possibly due to rounding, one of the amounts at risk was zero.
    }
    ...
    function parseOrder(uint[3] memory rawOrder) private constant returns(Order memory o) {
        o.orderHash = uint(keccak256(abi.encodePacked(this, rawOrder)));
    ...
    /**
    Adds funds to the sender's balance inside the smart contract. 
    This is the only payable method (and that is verified by t/interface.js).
    */
    function deposit() external payable {
        if (msg.value > 0) {
            ...
            emit LogBalanceChange(msg.sender, origAmount, newAmount);
        }
    }
    /**
     Deducts the sender's smart contract balance and sends it to their address with transfer. 
     This method attempts to withdraw the maximum up to the specified amount. 
     If the balance is less than this amount, the entire balance will be withdrawn.
    */
    ...
    /**
    Given a signed order, sets the filledAmounts value for this order to the order amount, ensuring that this order can no longer be traded on. 
    The order must be signed by the sender. 
    If the order is already expired, saves gas by not writing to filledAmounts.
    */
    function cancelOrder(uint[3] order, bytes32 r, bytes32 s, uint8 v) external {
        ...
        if (block.timestamp < o.expiry) {
            ...
            emit LogOrderCancel(msg.sender, o.matchId, o.orderHash);
        }
    }
    /**
    Given a maximum trade amount and a signed order, attempts to create a trade with the amount at risk up to the specified amount. 
    This will debit or credit the balances, and increase or decrease positions on the event specified in the order.
    See our whitepaper for full details. 
    In the event of an error, a LogTradeError will be logged (see the Trade Status section below).
    */
    function trade(uint amount, uint[3] order, bytes32 r, bytes32 s, uint8 v) external {
        Order memory o = parseOrder(order);

        if (!validateOrderParams(o)) {
            emit LogTradeError(msg.sender, o.addr, o.matchId, o.orderHash, uint16(Status.ORDER_MALFORMED));
            return;
        }

        if (!validateOrderSig(o, r, s, v)) {
            emit LogTradeError(msg.sender, o.addr, o.matchId, o.orderHash, uint16(Status.ORDER_BAD_SIG));
            return;
        }
        ...
        if (t.status != Status.OK) {
            emit LogTradeError(msg.sender, o.addr, o.matchId, o.orderHash, uint16(t.status));
            return;
        }

        // Modify storage to reflect trade:

        Match m = matches[o.matchId];
        ...
        emit LogTrade(msg.sender, o.addr, o.matchId, o.orderHash, o.direction, o.price, t.longAmount, t.newLongPosition, t.shortAmount, t.newShortPosition);
    }
    /**
    Given a signed finalization message and a match ID, finalizes the match and claims the sender's winnings for this match by decreasing the sender's position and crediting the sender's balance. 
    This is an irreversible operation. 
    After finalization, the signed finalization message is no longer required for claims.
    */
    function claim(uint matchId, uint8 finalPrice, bytes32 r, bytes32 s, uint8 v) external {
        ...
        if (m.finalized) {
            ...
            uint messageHash = uint(keccak256(abi.encodePacked(this, matchId, finalPrice)));
        ...
        emit LogClaim(msg.sender, matchId, uint(delta));
    }
    /**
    Given a match ID, if a match has not been finalized and its funds recovery timeout period has elapsed, finalizes the match at its cancellation price.
    */
    ...
    function adjustBalance(address addr, int delta) private {
        ...
        emit LogBalanceChange(addr, origAmount, newAmount);
    }
